
export const lionLeader = [
    {
        organization: '멋쟁이사자처럼 12기 대표',
        name: '김현수',
        department: '컴퓨터공학과',
        desc: '서버 / QR 스탬프 지도 / 유형 테스트',
        img: require('../../assets/img/etc/lion01.avif'),
    },
    {
        organization: '멋쟁이사자처럼 12기 부대표',
        name: '송지연',
        department: '서비스・디자인공학과',
        desc: '팜플렛 / QR 스탬프 지도 / 부스 / 유형 테스트 / 푸드트럭 영양성분 / 축제 지도',
        img: require('../../assets/img/etc/lion02.avif'),
    },
]

export const design = {
    organization: '멋쟁이사자처럼 12기',
    name: '권하영',
    department: '서비스・디자인공학과',
    desc: '랜딩 / 메인 / 회원가입 / 로그인 / 로딩 / 타임테이블 / 관리자 모드 / 굿즈 사전예약 / 후원사 / 만든이들',
    img: require('../../assets/img/etc/lion03.avif'),
}

export const frontEnd = [
    {
        organization: '멋쟁이사자처럼 12기',
        name: '박예인',
        department: 'AI융합학부',
        desc: '타임테이블 / 배너',
        img: require('../../assets/img/etc/lion04.avif'),
    },
    {
        organization: '멋쟁이사자처럼 12기',
        name: '박유채',
        department: '서비스・디자인공학과',
        desc: '회원가입 / 로그인 / 관리자 모드',
        img: require('../../assets/img/etc/lion05.avif'),
    },
    {
        organization: '멋쟁이사자처럼 12기',
        name: '우지윤',
        department: 'AI융합학부',
        desc: '팜플렛 / 유형 테스트 / 타임테이블 / 굿즈 사전예약 / 만든이들',
        img: require('../../assets/img/etc/lion06.avif'),
    },
    {
        organization: '멋쟁이사자처럼 12기',
        name: '윤나경',
        department: 'AI융합학부',
        desc: '후원사 / 만든이들 / 로딩',
        img: require('../../assets/img/etc/lion07.avif'),
    },
    {
        organization: '멋쟁이사자처럼 12기',
        name: '이현진',
        department: 'AI융합학부',
        desc: 'QR 스탬프 지도 / 부스 / 푸드트럭 영양성분 / 축제 지도',
        img: require('../../assets/img/etc/lion08.avif'),
    },
    {
        organization: '멋쟁이사자처럼 12기',
        name: '정새영',
        department: '미디어커뮤니케이션학과',
        desc: '랜딩 / 메인 / 회원가입 / 로그인 / 관리자 모드',
        img: require('../../assets/img/etc/lion09.avif'),
    },
    {
        organization: '멋쟁이사자처럼 12기',
        name: '최수진',
        department: '컴퓨터공학과',
        desc: 'QR 스탬프 지도 / 부스 / 푸드트럭 영양성분 / 축제 지도',
        img: require('../../assets/img/etc/lion10.avif'),
    },
]

export const backEnd = [
    {
        organization: '멋쟁이사자처럼 12기',
        name: '서지우',
        department: 'AI융합학부',
        desc: '관리자 모드',
        img: require('../../assets/img/etc/lion11.avif'),
    },
    {
        organization: '멋쟁이사자처럼 12기',
        name: '이예령',
        department: '컴퓨터공학과',
        desc: '회원가입 / 로그인',
        img: require('../../assets/img/etc/lion12.avif'),
    }
]

export const sswuleader = [
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '임수빈',
        department: '법학부'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '박세은',
        department: '법학부'
    }
]

export const festivalleader = {
    organization: "제36대 총학생회 ‘여일하게'",
    name: '피정빈',
    department: '경제학과'
}

export const festivalmemeber = [
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '홍은영',
        department: '정치외교학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '엄채연',
        department: '서양화과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '최희영',
        department: '소비자생활문학산업학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '조현서',
        department: '국어국문학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '오나연',
        department: '문화예술경영학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '이은재',
        department: '인문융합예술계열'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '임채민',
        department: '문화예술경영학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '김나현',
        department: '뷰티산업학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '정유민',
        department: '일본어문・문화학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '이경민',
        department: '일본어문・문화학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '김세진',
        department: '법학부'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '정다희',
        department: '현대실용음악학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '박소유',
        department: '융합보안공학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '신지수',
        department: '국어국문학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '박유채',
        department: '서비스・디자인공학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '허수빈',
        department: '미디어커뮤니케이션학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '최인애',
        department: '미디어커뮤니케이션학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '전여원',
        department: '뷰티산업학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '박수아',
        department: '경영학부'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '이선민',
        department: '법학부'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '윤수민',
        department: '디자인과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '김미연',
        department: '사회과학계열'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '홍영인',
        department: '문화예술경영학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '신채연',
        department: '법학부'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '이현지',
        department: '융합보안공학과'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '김윤서',
        department: '사회과학계열'
    },
    {
        organization: "제36대 총학생회 ‘여일하게'",
        name: '손예빈',
        department: '바이오식품공학과'
    },
    {
        organization: "2024 수정대동제 축제기획단",
        name: '김채연',
        department: '디자인과'
    },
    {
        organization: "2024 수정대동제 축제기획단",
        name: '민서현',
        department: '미디어커뮤니케이션학과'
    },
    {
        organization: "2024 수정대동제 축제기획단",
        name: '이솔희',
        department: '디자인과'
    },
]
